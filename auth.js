    ...
    ${selected ? `
      <section class="intake-map" id="intakeMap" tabindex="-1">
        ...
        <ol class="intake-section-list">
          ${checklist.map((section, index) => `<li><span>${index + 1}</span><div><h3>${escapeHtml(section.title)}</h3><p>${escapeHtml(section.summary)}</p><small><strong class="manual-location-label">${section.id === 'additionalSupport' ? 'Service path:' : 'Manual location:'}</strong> ${policyReferenceLinks(section.policy)}</small></div></li>`).join('')}
        </ol>
