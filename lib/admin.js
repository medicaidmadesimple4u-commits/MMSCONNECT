...
const allowedStagingOrigin = 'https://mms-connect-env-staging-mms-navigators.vercel.app';
...
export function requireAllowedOrigin(request, response) {
  ...
  if (origin === allowedProductionOrigin || origin === allowedStagingOrigin || stagingOriginPattern.test(origin)) return origin;
...
export async function requireSignedInProfile(request) {
  ...
  const profiles = await serviceRequest(`/rest/v1/profiles?select=id,first_name,last_name,organization_name,organization_id,account_type,status&id=eq.${encodeURIComponent(user.id)}&limit=1`);
...
export async function requireReferralUser(request) {
  const context = await requireSignedInProfile(request);
  if (!['agency', 'facility', 'staff', 'administrator'].includes(context.profile.account_type)) {
    const error = new Error('An approved organization or MMS staff account is required.');
    error.status = 403;
    throw error;
  }
  if (!context.profile.organization_id) {
    const error = new Error('Your organization directory access is not ready. Contact an MMS administrator.');
    error.status = 403;
    throw error;
  }
  return context;
